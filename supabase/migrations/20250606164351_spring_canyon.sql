/*
  # Fix Word Generation Migration

  1. Database Cleanup
    - Remove all numbered words (word_1, word_2, etc.)
    - Clean up related data (translations, relationships, trends)

  2. Generate Meaningful Words
    - Insert 200 real English words with proper translations
    - Add Korean and Japanese translations
    - Create realistic trending data
    - Establish word relationships

  3. Data Quality
    - All words are meaningful vocabulary
    - Proper categorization (General, Technology, Nature, etc.)
    - Realistic bookmark and view counts
*/
...
BEGIN
  ...
  FOR i IN 1..array_length(word_list, 1) LOOP
    ...
    -- Get the word ID using a different variable name
    SELECT id INTO current_word_id FROM words WHERE word = word_list[i];
    
    -- Insert Korean translation
    INSERT INTO word_translations (word_id, language, translation)
    VALUES (current_word_id, 'ko', korean_translations[i])
    ON CONFLICT (word_id, language) DO UPDATE SET
      translation = EXCLUDED.translation;
    
    -- Insert Japanese translation
    INSERT INTO word_translations (word_id, language, translation)
    VALUES (current_word_id, 'ja', japanese_translations[i])
    ON CONFLICT (word_id, language) DO UPDATE SET
      translation = EXCLUDED.translation;
...
WHERE (w1.word = 'ability' AND w2.word = 'talent')
   ...
   OR (w1.word = 'camera' AND w2.word = 'laptop')
   OR (w1.word = 'family' AND w2.word = 'parent')
   OR (w1.word = 'health' AND w2.word = 'safety')
   OR (w1.word = 'nature' AND w2.word = 'garden')
   OR (w1.word = 'school' AND w2.word = 'educate')
   OR (w1.word = 'laptop' AND w2.word = 'technology')
   OR (w1.word = 'wisdom' AND w2.word = 'genius')
...
WHERE (w1.word = 'ability' AND w2.word = 'talent')
   ...
   OR (w1.word = 'enhance' AND w2.word = 'advance')
   OR (w1.word = 'wealth' AND w2.word = 'capital')
ON CONFLICT (source_word_id, target_word_id, relationship_type) DO NOTHING;
...
FROM words w1, words w2
WHERE (w1.word = 'advance' AND w2.word = 'defeat')
   OR (w1.word = 'success' AND w2.word = 'failure')
   OR (w1.word = 'indoor' AND w2.word = 'outdoor')
   OR (w1.word = 'enable' AND w2.word = 'unable')
ON CONFLICT (source_word_id, target_word_id, relationship_type) DO NOTHING;